    ...
    public partial class S_OPDN : Form
    {
        ...
        void m_oWorker_DoWork(object sender, DoWorkEventArgs e)
        {
            try
            {
                ...
                for (int i = 0; i < dgv_S_OPDN.RowCount; i++)
                {
                    try
                    {
                        bool blnSync = false;
                        ...
                        sQuery += " Where T0.U_SHOPID = '" + dgv_S_OPDN.Rows[i].Cells["ShopID"].Value.ToString().Trim() + "'";
                        ...
                        TransLog.traceService(" Transaction Type : " + dgv_S_OPDN.Rows[i].Cells["Scenario"].Value.ToString().Trim());
                        TransLog.traceService(" Transaction Key : " + dgv_S_OPDN.Rows[i].Cells["Source_Key"].Value.ToString().Trim());
                        if (oGetCompany != null)
                        {
                            TransLog.traceService("Company DB : " + oGetCompany.CompanyDB);
                            if (oGetCompany.Connected)
                            {
                                Singleton.obj_S_OPDN.Sync((dgv_S_OPDN.Rows[i].Cells["Source_Key"].Value.ToString().Trim()), TransType.A, oGetCompany, strInterface, strWareHouse, strValues, htCCDet);

                                string strQuery = "Select Status,Remarks From dbo.Z_OTXN Where Scenario = '" + dgv_S_OPDN.Rows[i].Cells["Scenario"].Value.ToString().Trim() + "' ";
                                strQuery += " And S_DocNo = '" + dgv_S_OPDN.Rows[i].Cells["Source_Key"].Value.ToString().Trim() + "' ";
                                ...
                                if (oStatus != null && oStatus.Rows.Count > 0)
                                {
                                    ...
                                    {
                                        blnSync = true;
                                        dgv_S_OPDN.Rows[i].Cells["Remarks"].Value = string.Empty;
                                    }
                                    else
                                    {
                                        blnSync = false;
                                        dgv_S_OPDN.Rows[i].Cells["Remarks"].Value = oStatus.Rows[0][1].ToString();
                                    }
                                }
                                else
                                {
                        ...
                        if (blnSync)
                        {
                            ...
                            dgv_S_OPDN.Rows[i].Cells["Image"].Value = image;
                        }
                        else
                        {
        ...
        private void LoadAll()
        {
            try
            {
                ...
                LoadShopXML();
                string str_M_S_ORIN = "Exec Armada_Service_M_S_OPDN_s '" + oFDate.ToString("yyyyMMdd") + "','" + oTDate.ToString("yyyyMMdd") + "','" + strShopXML + "'";
                oDS_M_S_ORIN = Armada_Sync.Singleton.objSqlDataAccess.ExecuteReader(str_M_S_ORIN, strInterface);
                dgv_S_OPDN.DataSource = oDS_M_S_ORIN;
                loadError();
            }
            catch (Exception ex)
            {   
        ...
        private void loadError()
        {
            try
            {
                for (int i = 0; i < dgv_S_OPDN.RowCount; i++)
                {
                    if (dgv_S_OPDN.Rows[i].Cells[1].Value.ToString() == "-1")
                    {
